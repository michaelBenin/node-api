...
gulp.task('js-lint-gulp', function lintGulp() {
  ...
  gulpConf.rules['import/no-extraneous-dependencies'] = 'off';
  gulpConf.rules['node/no-unpublished-require'] = 0;
...
gulp.task('js-lint-test', function lintTests() {
  ...
  baseConfig.rules['import/no-extraneous-dependencies'] = 'off';
  baseConfig.rules['node/no-unpublished-require'] = 0;
...
gulp.task('js-lint-gulp-fix', function lintGulp() {
  const confFix = cloneDeep(conf);
  confFix.fix = true;
  confFix.rules['import/no-extraneous-dependencies'] = 'off';
  confFix.rules['node/no-unpublished-require'] = 0;
  return gulp.src('gulpfile.js/**/*.js')
    .pipe(eslint(confFix))
    .pipe(eslint.format())
